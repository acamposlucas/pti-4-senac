	...
	async function handleFormSubmit(e: FormEvent) {
		...
		function resetForm() {
			formData.forEach((_, key) => {
				const input = loginFormRef.current?.elements.namedItem(key as string) as HTMLInputElement | null;
				if (input) {
					input.value = "";
				}
			});
	...
	return (
		<>
			<Form onSubmit={handleFormSubmit} ref={loginFormRef}>
				...
				<Button className="mt-3 d-block mx-auto" type="submit" variant="primary">
					Entrar
				</Button>
			</Form>
			<div className="mt-3 text-center">
				<Link style={{ textDecoration: "none" }} to="/cadastro">
					Não possui cadastro? <br /> Clique aqui!
				</Link>
			</div>
		</>
	);
